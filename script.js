(() => {
  ...
  function applyAboutMotion(progress) {
    ...
    /*
      v156:
      - mantengo la misma sección
      - hago que el sello derecho y el gráfico inferior entren antes,
        para que ya estén visibles cuando el texto central aparece.
    */
    const graphicProgress = easeInOut(Math.max(0, Math.min(1, safeProgress / 0.44)));

    const sealX = (1 - graphicProgress) * Math.max(320, viewportWidth * 0.34);
    const sealY = Math.sin(graphicProgress * Math.PI) * 4;
    const badgeY = (1 - graphicProgress) * 220 - graphicProgress * 24;

    const sealOpacity = Math.max(0, Math.min(1, safeProgress / 0.08));
    const badgeOpacity = Math.max(0, Math.min(1, safeProgress / 0.08));
    ...
    aboutCollage.style.setProperty("--seal-rotate", `${(-7 + graphicProgress * 6).toFixed(2)}deg`);
    ...
    aboutCollage.style.setProperty("--badge-rotate", `${(-2 + graphicProgress * 2).toFixed(2)}deg`);
  ...
  function initAboutCursorColor() {
    const targets = Array.from(document.querySelectorAll(".about-manifesto-title, .about-manifesto-copy p"));
    if (!targets.length || reducedMotion || !window.matchMedia("(pointer: fine)").matches) return;

    targets.forEach((target) => {
      target.classList.add("about-manifesto-spray-target");

      const paintLayer = document.createElement("span");
      paintLayer.className = "about-manifesto-text-paint";
      paintLayer.setAttribute("aria-hidden", "true");
      paintLayer.innerHTML = target.innerHTML;
      target.appendChild(paintLayer);
      ...
      function spray(x, y) {
        ...
        for (let i = 0; i < coreBursts; i += 1) {
          ...
          const radius = Math.pow(Math.random(), 0.72) * 30;
          const size = 2 + Math.random() * 8.5;
          const px = x + Math.cos(angle) * radius;
          const py = y + Math.sin(angle) * radius;

          maskCtx.globalAlpha = 0.28 + Math.random() * 0.32;
          maskCtx.beginPath();
        ...
        for (let i = 0; i < mistBursts; i += 1) {
          const angle = Math.random() * Math.PI * 2;
          const radius = Math.pow(Math.random(), 0.94) * 54;
          const size = 0.8 + Math.random() * 2.6;
          const px = x + Math.cos(angle) * radius;
          const py = y + Math.sin(angle) * radius;

          maskCtx.globalAlpha = 0.05 + Math.random() * 0.12;
          maskCtx.beginPath();
          maskCtx.arc(px, py, size, 0, Math.PI * 2);
          maskCtx.fill();
        }
  ...
  function initNarrativeDogSpray() {
    const visual = document.querySelector("[data-narrative-spray]");
    if (!visual || reducedMotion || !window.matchMedia("(pointer: fine)").matches) return;

    const orangeLayer = visual.querySelector(".narrative-spray-visual__orange");
    const baseLayer = visual.querySelector(".narrative-spray-visual__base");
    if (!orangeLayer || !baseLayer) return;
    ...
    function commitMask() {
      ...
      orangeLayer.style.webkitMaskImage = maskUrl;
      orangeLayer.style.maskImage = maskUrl;
    }

    function requestCommit() {
      if (!rafId) rafId = requestAnimationFrame(commitMask);
    }
    ...
    function sprayPoint(x, y, intensity = 1) {
      const coreBursts = Math.round(28 * intensity);
      const mistBursts = Math.round(95 * intensity);
      ...
      for (let i = 0; i < coreBursts; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const radius = Math.pow(Math.random(), 0.7) * 34;
        const size = 2.2 + Math.random() * 9.5;
        const px = x + Math.cos(angle) * radius;
        const py = y + Math.sin(angle) * radius;

        maskCtx.globalAlpha = 0.26 + Math.random() * 0.36;
      ...
      for (let i = 0; i < mistBursts; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const radius = Math.pow(Math.random(), 0.95) * 66;
        const size = 0.7 + Math.random() * 3.4;
        const px = x + Math.cos(angle) * radius;
        const py = y + Math.sin(angle) * radius;

        maskCtx.globalAlpha = 0.045 + Math.random() * 0.13;
        maskCtx.fillStyle = "#fff";
        maskCtx.beginPath();
        maskCtx.arc(px, py, size, 0, Math.PI * 2);
        maskCtx.fill();
    ...
    function sprayLine(x, y) {
      ...
      for (let i = 1; i <= steps; i += 1) {
        const t = i / steps;
        sprayPoint(lastX + dx * t, lastY + dy * t, 0.85);
      }
    ...
    visual.addEventListener("pointerenter", update);
    ...
    baseLayer.addEventListener("load", resizeMask, { once: true });

    resizeMask();
  }


  function initBioHiddenPhraseReveal() {
    const phrase = document.querySelector("[data-hidden-phrase]");
    if (!phrase || reducedMotion || !window.matchMedia("(pointer: fine)").matches) return;

    const source = phrase.querySelector(".bio-hidden-phrase__text");
    if (!source) return;

    const paintLayer = document.createElement("span");
    paintLayer.className = "bio-hidden-phrase__paint";
    paintLayer.setAttribute("aria-hidden", "true");
    paintLayer.innerHTML = source.innerHTML;
    phrase.appendChild(paintLayer);
    ...
    function commitMask() {
      rafId = 0;
      const maskUrl = `url(${maskCanvas.toDataURL("image/png")})`;
      paintLayer.style.webkitMaskImage = maskUrl;
      paintLayer.style.maskImage = maskUrl;
    }
    ...
    function revealPoint(x, y, intensity = 1) {
      const coreBursts = Math.round(38 * intensity);
      const mistBursts = Math.round(130 * intensity);
      ...
      for (let i = 0; i < coreBursts; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const radius = Math.pow(Math.random(), 0.66) * 42;
        const size = 3 + Math.random() * 12;
        const px = x + Math.cos(angle) * radius;
        const py = y + Math.sin(angle) * radius;

        maskCtx.globalAlpha = 0.34 + Math.random() * 0.42;
        maskCtx.beginPath();
        maskCtx.arc(px, py, size, 0, Math.PI * 2);
        maskCtx.fill();
      }
      ...
      for (let i = 0; i < mistBursts; i += 1) {
        const angle = Math.random() * Math.PI * 2;
        const radius = Math.pow(Math.random(), 0.92) * 74;
        const size = 0.9 + Math.random() * 3.8;
        const px = x + Math.cos(angle) * radius;
        const py = y + Math.sin(angle) * radius;

        maskCtx.globalAlpha = 0.05 + Math.random() * 0.15;
        maskCtx.beginPath();
        maskCtx.arc(px, py, size, 0, Math.PI * 2);
        maskCtx.fill();
      }
    ...
    function revealLine(x, y) {
      if (!hasLast) {
        revealPoint(x, y, 1.12);
        lastX = x;
        lastY = y;
        hasLast = true;
        requestCommit();
      ...
      for (let i = 1; i <= steps; i += 1) {
        const t = i / steps;
        revealPoint(lastX + dx * t, lastY + dy * t, 0.72);
      }
    ...
    phrase.addEventListener("pointerenter", update);
    phrase.addEventListener("pointermove", update);
    phrase.addEventListener("pointerleave", () => {
      hasLast = false;
    });

    window.addEventListener("resize", resizeMask);
    window.addEventListener("load", resizeMask);
    resizeMask();
  }
  ...
  function initVandalAboutMotion() {
    const section = document.querySelector("[data-behind-vandal-intro]");
    const sticky = section ? section.querySelector(".behind-vandal-intro__sticky") : null;
    ...
    function update() {
      ...
      const progress = clampValue(-rect.top / travel);

      const enter = ease(progress / 0.34);
      const exit = ease((progress - 0.54) / 0.24);

      const alpha = clampValue(enter * (1 - exit * 0.92));
      const titleY = 16 - enter * 16 + exit * 8;
      const titleScale = 0.96 + enter * 0.04 - exit * 0.02;

      sticky.style.setProperty("--behind-alpha", alpha.toFixed(3));
      sticky.style.setProperty("--behind-y", `${titleY.toFixed(2)}vh`);
      sticky.style.setProperty("--behind-scale", titleScale.toFixed(3));
    }
  ...
  initVandalAboutMotion();
  initAboutCursorColor();
  initNarrativeDogSpray();
  initBioHiddenPhraseReveal();
  initVandalAugustAutoMarquee();
  initHoodieVideoLoop();
